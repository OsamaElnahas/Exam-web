...
var questionsTemplate = [
  ...
  {
    question:
      "In JavaScript, which method is used to write text to the HTML document?",
    correct_answer: "document.write()",
    answers: ["console.log()", "document.write()", "alert()", "print()"],
    choosen_answer: "",
    marked: false,
  },
  {
...
if (currentUser.exam.questions.length === 0) {
  currentUser.exam.questions = JSON.parse(JSON.stringify(questionsTemplate));
  saveCurrentUser();
}
...
function showTimeUpModal() {
  ...
  setTimeout(function () {
    timeUpModal.querySelector("div").classList.add("show-modal");
  }, 50);
...
function showTimer() {
  ...
  for (var i = 0; i < timer.length; i++) {
    timer[i].textContent = format(h) + " : " + format(m) + " : " + format(s);
  }
